...
const DB_PATH = path.join(__dirname, "../data/prison_config.json");
...
// Ensure the prison system is fully setup with roles and channels
async function setupPrison(guild) {
    ...
    if (!jailRole) {
        jailRole = await guild.roles.create({
            name: "Jail",
            color: "#010101",
            reason: "interX Prison System Role"
    ...
    guild.channels.cache.forEach((ch) => {
        if (ch.parentId && (ch.parent && ch.parent.name.toLowerCase() === "prison")) return;
        try {
            if (ch.manageable) {
    ...
    let prisonCategory = guild.channels.cache.find(c => c.type === ChannelType.GuildCategory && c.name === "Prison");
    if (!prisonCategory) {
    ...
    // Create Text Channel
    let prisonChat = guild.channels.cache.find(c => c.name === "prison-chat" && c.parentId === prisonCategory?.id);
    if (!prisonChat && prisonCategory) {
        prisonChat = await guild.channels.create({
            name: "prison-chat",
    ...
    let prisonVoice = guild.channels.cache.find(c => c.name === "Prison VC" && c.parentId === prisonCategory?.id);
    if (!prisonVoice && prisonCategory) {
    ...
    return { jailRole, prisonCategory, prisonChat, prisonVoice };
}
...
async function arrestUser(guild, userId, reason, client) {
    ...
    const { jailRole, prisonChat } = await setupPrison(guild);
    if (!jailRole) return;
    ...
    const embed = new EmbedBuilder()
        ...
        .setDescription(
            ...
            `All your previous roles have been stripped. All channels have been locked. You are now restricted solely to this prison block. Wait for administrative judgment.`
        )
    ...
    if (prisonChat) {
        prisonChat.send({ content: `<@${userId}> ||<@${guild.ownerId}>||`, embeds: [embed] }).catch(() => {});
    }
}
...
module.exports = {
    ...
    data: new SlashCommandBuilder()
        .setName("prison")
        .setDescription("Manage the prison automation system")
        .addSubcommand(s => s.setName("enable").setDescription("Enable the automatic prison trap"))
        .addSubcommand(s => s.setName("disable").setDescription("Disable the automatic prison trap"))
        .addSubcommand(s => s.setName("setup").setDescription("Generate the prison channels manually")),
    ...
    async execute(message, args, client) {
        ...
        if (args[0] === "setup") {
            const msg = await message.reply("⚙️ Constructing Prison layout...");
            await setupPrison(message.guild);
            return msg.edit("✅ Prison structural format built dynamically!");
        }
        ...
        if (args[0] === "enable") {
            ...
            saveConfig(config);
            return message.reply({ embeds: [new EmbedBuilder().setColor("#00FF00").setTitle("🚨 Prison Trap Activated").setDescription("The trap is set! Anyone touching the server name, channels, or roles will be instantly thrown into `#prison-chat`.")] });
        }

        if (args[0] === "disable") {
            config[guildId] = { enabled: false };
            saveConfig(config);
            return message.reply({ embeds: [new EmbedBuilder().setColor("#FF0000").setTitle("❌ Prison Trap Deactivated").setDescription("Automated jailing has been paused.")] });
        }

        return message.reply({ embeds: [new EmbedBuilder().setColor("#2B2D31").setDescription("Usage: `!prison setup`, `!prison enable`, `!prison disable`\n*(Type !prison enable to start automatically arresting structural griefers!)*")] });
    },
    ...
    async slashExecute(interaction, client) {
        ...
        if (sub === "setup") {
            await interaction.reply("⚙️ Constructing Prison layout...");
            await setupPrison(interaction.guild);
            return interaction.editReply("✅ Prison structural format built dynamically!");
        }

        if (sub === "enable") {
            config[guildId] = { enabled: true };
            saveConfig(config);
            return interaction.reply({ embeds: [new EmbedBuilder().setColor("#00FF00").setTitle("🚨 Prison Trap Activated").setDescription("The trap is set! Anyone touching the structural integrity will be jailed.")] });
        }

        if (sub === "disable") {
            config[guildId] = { enabled: false };
            saveConfig(config);
            return interaction.reply({ embeds: [new EmbedBuilder().setColor("#FF0000").setTitle("❌ Prison Trap Deactivated")] });
        }
    },
    ...
    attachEvents(client) {
        ...
        console.log("🔒 [Prison Trap] Sentinels Activated");
