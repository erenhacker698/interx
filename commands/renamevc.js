    ...
    async execute(message, args) {
        ...
        if (args.length < 1) {
            return message.reply({
                components: [V2.container([V2.section(["⚠️ MISSING NAME", "> **Usage:** `!renamevc <new_name>`"], botAvatar)])]
            });
        }
        ...
        if (!target || target.type !== ChannelType.GuildVoice) {
            return message.reply({
                components: [V2.container([V2.section(["❌ INVALID CHANNEL", "> Target must be a voice channel you are in or have specified."], botAvatar)])]
            });
        }

        if (!newName) {
            return message.reply({
                components: [V2.container([V2.section(["⚠️ NAME REQUIRED", "> Please provide the new name."], botAvatar)])]
            });
        }
